...
async def async_setup_entry(
    ...
    for serial_number, device_info in registered_devices.items():
        ...
        if has_action_buttons:
            ...
            _LOGGER.debug("Tracking device %s as having action buttons", serial_number[-6:])
        ...
        if orphaned_repair:
            ...
            if serial_number in devices_with_button_entities:
                if device_type != "ew_receiver":
                    _LOGGER.debug("Skipping device %s - action button entities already created during initial setup", serial_number[-6:])
                    return
                else:
                    ...
                    if not button_entities:
                        _LOGGER.debug("Skipping EW-Receiver %s - no button entities configured", serial_number[-6:])
                        return
                    else:
                        _LOGGER.debug("Retrying EW-Receiver %s - found %d entities to create", serial_number[-6:], len(button_entities))
    ...
    async def _handle_button_device_added(event):
        ...
        if serial_number in devices_with_button_entities:
            _LOGGER.debug("Skipping button-specific event for %s - entities already created", serial_number[-6:])
            return
            ...
            if success:
                ...
                if press_type == "short":
                    self.hass.bus.async_fire("eldat_button_short_press", {
                        ...
                        "is_active": self._is_long_press_active
                    })
                elif press_type == "long_start":
                    self.hass.bus.async_fire("eldat_button_long_press_start", {
                        "device_id": self._serial_number,
                        "entity_id": self.entity_id,
                        "subtype": self._action,
                        "action": self._action,
                    ...
                    self.hass.bus.async_fire("eldat_button_long_press_end", {
                        "device_id": self._serial_number,
                        "entity_id": self.entity_id,
                        "subtype": self._action,
                        "action": self._action,
                ...
                self.hass.bus.async_fire("eldat_button_pressed", {
                    "device_id": self._serial_number,
                    "entity_id": self.entity_id,
    ...
    async def _execute_simple_press(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                # Fire button press event for automation
                self.hass.bus.async_fire("eldat_button_pressed", {
                    "device_id": self._serial_number,
                    "entity_id": self.entity_id,
                    "press_type": "short",
...
class EldatRemoveButton(EldatEntity, ButtonEntity):
    ...
    def __init__(self, coordinator: EldatCoordinator, serial_number: str, device_info: dict):
        ...
        # Enhanced button description
        if device_info.get("added_manually", False):
            self._attr_extra_state_attributes = {
                "description": f"Entferne {device_type} '{device_name}' vollständig",
            ...
            self._attr_extra_state_attributes = {
                "description": f"Entferne {device_type} '{device_name}'",
                "device_type": device_type,
                "added_manually": False,
                "can_remove": True,
                "warning": "Gerät wurde automatisch erkannt"
            }
        
    async def async_press(self) -> None:
        """Handle device removal with enhanced logging and cleanup."""
        try:
            serial_number = self._serial_number
            device_info = self.coordinator.get_device(serial_number)
            if not device_info:
                _LOGGER.error("Device %s not found for removal", serial_number[-8:])
                return
                
            device_name = device_info.get('name', serial_number[-8:])
            device_type = device_info.get('type', 'unknown')
            is_manual = device_info.get('added_manually', False)
            
            _LOGGER.info("🗑️ Starting device removal: %s (%s) - Type: %s, Manual: %s", 
                        device_name, serial_number[-8:], device_type, is_manual)
            ...
            if success:
                _LOGGER.info("✅ Device '%s' (%s) removed successfully via button", device_name, serial_number[-8:])
                
                # Fire notification event for UI
                self.hass.bus.async_fire(
                    "eldat_device_removal_success",
                    {
                        "serial_number": serial_number,
                        "device_name": device_name,
                        "device_type": device_type,
                        "removal_method": "button",
                        "was_manual": is_manual
                    }
                )
            else:
                _LOGGER.error("❌ Failed to remove device '%s' (%s) via button", device_name, serial_number[-8:])
            ...
            _LOGGER.error("❌ Exception during device removal: %s", e)
...
class DeviceRemoveButton(ButtonEntity, EldatEntity):
    ...
    async def async_press(self) -> None:
        ...
        try:
            _LOGGER.info("🗑️ Remove button pressed for device: %s", self._serial_number[-8:])
