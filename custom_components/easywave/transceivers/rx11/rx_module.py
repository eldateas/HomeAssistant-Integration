...
class RxModule:
    ...
    def _reset_rx_state(self):
        """Reset RX state machine for clean packet reception.
        
        This should be called:
        - On initial connection
        - After protocol errors (corrupt packets, invalid stuffing, etc.)
        - NOT for hardware errors which require full reconnect
        """
        self._rx_sop = False
        self._rx_raw_buffer = bytearray()
        self._rx_stuffing = False
        _LOGGER.debug("RX state reset - ready for next packet")
    ...
    def _write_to_buffer(self, irp: IRP, req_str: str):
        ...
        try:
            ...
            if not self._shutdown_requested:
                ...
                # Trigger reconnect on write errors
                self._reconnect()
        except Exception as e:
            if not self._shutdown_requested:
        ...
        while not self._shutdown_requested:
            try:
                ...
                try:
                    ...
                    if not self._shutdown_requested:
                        error_str = str(e)
                        # Log only as DEBUG to reduce log spam - coordinator handles user notification
                        _LOGGER.debug("RX11 USB disconnected - reconnect will be initiated")
    ...
    def _process_received_byte(self, byte: int):
        ...
        if byte == SOP:
            if self._rx_sop:
                # Unexpected SOP - previous packet was incomplete, discard and start fresh
                # This is a protocol error, not a hardware error - don't reconnect!
                _LOGGER.warning("Unexpected SOP while already in packet (len=%d) - discarding incomplete packet", 
                              len(self._rx_raw_buffer) - 1)
                # Reset buffer but keep the new SOP
                self._rx_raw_buffer = bytearray([SOP])
                self._rx_stuffing = False
                # Don't return - continue with SOP handling below
            self._rx_sop = True
            self._rx_stuffing = False
            return
        ...
        if byte == EOP:
            if self._rx_stuffing:
                # EOP while expecting stuffed byte - protocol error, not hardware error
                _LOGGER.warning("Unexpected EOP while waiting for stuffed byte - discarding packet")
                self._reset_rx_state()
                return
            # Debug: Log complete packet before processing
            if self.debug:
                _LOGGER.debug("Complete packet received (len=%d): %s", len(self._rx_raw_buffer), self._rx_raw_buffer.hex())
        ...
        if len(self._rx_raw_buffer) >= 128:
            _LOGGER.warning("Packet too large (>128 bytes) - discarding and resetting RX state")
            # Reset packet state but don't kill connection - this is a protocol error
            self._reset_rx_state()
    ...
    def _process_complete_packet(self):
        ...
        if not success:
            # Check if we're in startup tolerance period
            in_startup = time.time() < self._startup_tolerance_until
            
            if in_startup:
                # During startup, silently discard invalid packets (noise from USB init)
                _LOGGER.debug("Discarding invalid packet during startup (len=%d, hex=%s)", 
                              len(raw_buffer), raw_buffer.hex()[:40])
    ...
    def _process_ipp(self, handle: int, raw_buffer: bytes):
        ...
        if self._tx_req_sent_size == 0:
            if in_startup:
                # During startup, unexpected IPPs can be leftovers from previous session
                _LOGGER.debug("Ignoring unexpected IPP during startup (handle=0x%04x)", handle)
                return
            # Outside startup: this is unexpected but not a hardware error
            _LOGGER.warning("Unexpected IPP received (handle=0x%04x) - no request pending", handle)
            return
        ...
        if handle == 0:
            # Zero handle is protocol error - fail the request but don't reconnect
            _LOGGER.warning("Received zero handle for request '%s' - failing request", req.req_str)
            req.icp = ICP(handle=0, result=ErrorCode.ERR_INVALID_REQUEST)
            req.signal()
            return
        ...
        if handle in self._req_pending:
            # Duplicate handle - protocol error, fail both requests but don't reconnect
            _LOGGER.warning("Duplicate handle 0x%04x - failing both requests", handle)
    ...
    def _process_icp(self, handle: int, icp: ICP, icp_byte_count: int, raw_buffer: bytes):
        ...
        if handle != 0:
            ...
            if not req:
                # Check if we're in the tolerance period after cancel_all_io or startup
                if time.time() < self._cancel_tolerance_until or in_startup:
                    # This is expected after restart/startup - device sends ICP for pre-restart operations
                    _LOGGER.debug("ICP for unknown handle 0x%04x during tolerance period (expected)", handle)
                    return
                
                # Outside tolerance period - log warning but don't reconnect
                # This is a protocol inconsistency, not a hardware failure
                _LOGGER.warning("ICP for unknown handle 0x%04x - ignoring (no matching request)", handle)
                return
        else:
            ...
            if not req:
                if in_startup:
                    _LOGGER.debug("Ignoring synchronous ICP during startup (no request pending)")
                    return
                _LOGGER.warning("Unexpected synchronous ICP - no request pending, ignoring")
                return
        ...
        if icp.result == ErrorCode.SUCCESS:
            ...
            # Error ICP with wrong length - fail this request but don't reconnect
            _LOGGER.warning("Error ICP has unexpected length %d (expected 3) for '%s' - failing request", 
                          icp_byte_count, req.req_str)
            req.icp = ICP(handle=handle, result=ErrorCode.ERR_SIZE_MISMATCH)
            req.signal()
            return
        ...
        if icp.result == ErrorCode.ERR_OUT_OF_QUEUE:
            if self._tx_req_queued_size < self.MAX_REQUEST_QUEUED:
                ...
                # Queue is full - fail the request but don't kill connection
                _LOGGER.warning("Cannot requeue request '%s' - queue full, failing request", req.req_str)
