    ...
    def create_device(
        ...
        **kwargs
        ...
        try:
            ...
            _LOGGER.info("🏭 Determined device_type: %s for %s", device_type, serial_number)
            
            if device_type == DeviceType.UNKNOWN:
                _LOGGER.warning("Unknown device type for device %s: device_info=%s, telegram_data=%s", 
                              serial_number, device_info, telegram_data)
                return None
            ...
            if device_type == DeviceType.EWNEO_SENSOR:
                ...
                _LOGGER.info("🏭 EWneo sensor types: %s for %s", sensor_types, serial_number)
            ...
            if device:
                _LOGGER.info("✅ Successfully created %s device %s", 
                            device.__class__.__name__, serial_number)
            else:
                _LOGGER.warning("⚠️ Factory function returned None for %s", serial_number)
            
            return device
        
        except Exception as e:
            _LOGGER.error(f"Failed to create device {serial_number}: {e}")
            return None
