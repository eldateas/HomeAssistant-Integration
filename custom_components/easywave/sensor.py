    ...
    for device in get_devices(entry):
        ...
        if entry_type == ENTRY_TYPE_TRANSMITTER:
            await async_setup_transmitter_entities(
                hass, entry, device, async_add_entities, "sensor"
            )
...
class EasywaveTransmitterLastButtonSensor(EasywaveTransmitterEntity, RestoreSensor):
    ...
    def icon(self) -> str:
        ...
        return _ICON_MAP_LAST_BUTTON.get(
            self._native_value or _BUTTON_STATE_RELEASED, "mdi:radiobox-marked"
        )
...
class EasywaveTransmitterButtonEnumSensor(EasywaveTransmitterEntity, RestoreSensor):
    """Per-button pressed/released enum sensor (HACS 1-button individual mode)."""
    ...
    def __init__(
        ...
        """Initialize."""
        letter = "abcd"[button_index]
        super().__init__(entry, device, f"button_{letter}")
        self._button_index = button_index
        self._switch_mode = switch_mode
        self._attr_translation_key = f"button_{letter}"
        self._attr_options = ["pressed", "released"]
        self._native_value: str | None = "released"
    ...
    @property
    def icon(self) -> str:
        """Return icon for pressed vs released."""
        if self._native_value == "pressed":
            return "mdi:gesture-tap-button"
        return "mdi:radiobox-blank"

    @callback
    def handle_telegram(self, event: ButtonPushEvent | ButtonReleaseEvent) -> None:
        """Update pressed/released for this button.

        Impulse: press → pressed, release → released.
        Permanent: each press toggles pressed ↔ released (HACS 0.6.x behavior).
        """
        if isinstance(event, ButtonPushEvent) and event.button == self._button_index:
            if self._switch_mode == TRANSMITTER_SWITCH_PERMANENT:
                self._native_value = (
                    "released" if self._native_value == "pressed" else "pressed"
                )
            else:
                self._native_value = "pressed"
            self.async_write_ha_state()
        elif (
            isinstance(event, ButtonReleaseEvent)
            and self._switch_mode == TRANSMITTER_SWITCH_IMPULSE
        ):
            self._native_value = "released"
            self.async_write_ha_state()


_STATE_ICONS_SWITCH: dict[str, str] = {
    "on": "mdi:light-switch",
    "off": "mdi:light-switch-off",
}
_STATE_ICONS_COVER: dict[str, str] = {
    "up": "mdi:window-shutter-open",
    "down": "mdi:window-shutter",
    "stop": "mdi:stop-circle",
    "on": "mdi:window-shutter-open",  # binary-style cover keys if reused
    "off": "mdi:window-shutter",
}


class EasywaveTransmitterStateSensor(EasywaveTransmitterEntity, RestoreSensor):
    """On/Off or Up/Down/Stop state sensor for op-2/op-3 transmitters."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize switch or cover3 state sensor."""
        self._mode = mode
        self._pair = pair
        if mode == "cover3":
            suffix = "state"
            self._attr_translation_key = "transmitter_state"
            self._attr_options = ["up", "down", "stop"]
            self._state_icons = _STATE_ICONS_COVER
            self._default_icon = "mdi:window-shutter"
            self._map = {0: "up", 1: "down", 2: "stop", 3: "stop"}
        elif pair == "cd":
            suffix = "state_cd"
            self._attr_translation_key = "state_cd"
            self._attr_options = ["on", "off"]
            self._state_icons = _STATE_ICONS_SWITCH
            self._default_icon = "mdi:light-switch"
            self._map = {2: "on", 3: "off"}
        elif pair == "ab":
    ...
    @property
    def icon(self) -> str:
        """Return icon matching current on/off or up/down/stop state (HACS UX)."""
        if self._native_value is None:
            return self._default_icon
        return self._state_icons.get(self._native_value, self._default_icon)

    @callback
    def handle_telegram(self, event: ButtonPushEvent | ButtonReleaseEvent) -> None:
        """Map button presses to semantic states."""
        if not isinstance(event, ButtonPushEvent):
            return
        value = self._map.get(event.button)
        if value is None or value not in (self._attr_options or ()):
            return
