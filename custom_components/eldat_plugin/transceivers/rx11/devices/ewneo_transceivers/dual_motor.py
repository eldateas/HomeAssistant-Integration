"""EWneo dual motor implementation for RX11 transceiver."""
...
class RX11EWneoDualMotor(BaseReceiver):
    """EWneo dual motor implementation for RX11 transceiver.
    
    Handles 2-channel EWneo motor devices for covers and blinds.
    """
    
    def __init__(self, *args, **kwargs):
        """Initialize EWneo dual motor."""
        super().__init__(*args, device_type=DeviceType.EWNEO_DUAL_MOTOR, 
                        subtype=DeviceSubtype.DUAL_MOTOR, **kwargs)
        self._motor_states = {1: "stop", 2: "stop"}
        self._positions = {1: None, 2: None}  # None=positionless mode, 0-100=position mode
        self._mode = 0  # Should always be 0 for dual motor
        
        # Motor status codes for each channel (bits 31-25 and 23-17)
        self._motor_status_codes = {1: 126, 2: 126}  # Default to stopped
        self._motor_status_texts = {1: "stopped", 2: "stopped"}
        
        # Position information for each channel
        self._current_positions = {1: 126, 2: 126}  # 0-100 or 126=unknown
        
        # Tilt information for each channel (bits 24 and 16)
        self._recent_tilts = {1: False, 2: False}  # Recently tilted to horizontal
        
        # Configuration flags (derived from status codes)
        self._runtime_measured = {1: False, 2: False}  # Can be inferred from status codes
        
        self._setup_operating_mode(**kwargs)
    
    def _setup_operating_mode(self, **kwargs) -> None:
        """Setup operating mode for dual motor."""
    ...
    def process_telegram(self, telegram_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process incoming telegram for EWneo dual motor."""
    ...
    def _handle_state_change(self, telegram_data: Dict[str, Any]) -> None:
        """Handle state change telegram with 5-byte EWB_RCV parsing for dual motor."""
        ...
        if raw_data and len(raw_data) >= 5:
            self._parse_ewneo_dual_motor_response(raw_data)
        else:
            ...
            if channel and 1 <= channel <= 2:
                ...
                if command and command != self._motor_states[channel]:
                    ...
                    _LOGGER.debug("EWneo dual motor %s channel %d: Command=%s, Position=%s (fallback)", 
    ...
    def _parse_ewneo_dual_motor_response(self, raw_data: bytes) -> None:
        """Parse 5-byte EWB_RCV response for EWneo dual motor.
        
        Format: 1 byte mode + 4 bytes state (big-endian)
        Mode 0: Summary for both motors (bits 31-25: motor1, bits 23-17: motor2)
        Mode 2: Full info for motor 1
        Mode 10: Full info for motor 2
        """
        if len(raw_data) < 5:
            _LOGGER.warning("EWneo dual motor %s: Invalid response length %d, expected 5 bytes", 
        ...
        if mode == 0:
            # Summary mode: Both motors with basic info
            self._parse_dual_motor_summary_mode(state_word)
        elif mode == 2:
            ...
            _LOGGER.warning("EWneo dual motor %s: Unsupported mode %d", 
                           self.serial_number[-6:], mode)
            
        self._mode = mode
    
    def _parse_dual_motor_summary_mode(self, state_word: int) -> None:
        """Parse dual motor state word in Mode 0 (summary for both motors)."""
        # Extract motor 1 status code (bits 31-25)
        motor1_status = (state_word >> 25) & 0x7F
        
        # Extract motor 1 tilt flag (bit 24)
        motor1_tilt = bool(state_word & (1 << 24))
        
        # Extract motor 2 status code (bits 23-17)
        motor2_status = (state_word >> 17) & 0x7F
        
        # Extract motor 2 tilt flag (bit 16)
    ...
    def _parse_individual_motor_full_info(self, channel: int, state_word: int) -> None:
        """Parse individual motor full information (Mode 2 for motor 1, Mode 10 for motor 2)."""
        ...
        if not hasattr(self, '_target_positions'):
            self._target_positions = {1: None, 2: None}
            self._tilt_measured = {1: False, 2: False}
            self._terrace_function = {1: False, 2: False}
            self._stored_position = {1: 0, 2: 0}
            self._auto_tilt = {1: False, 2: False}
        ...
        if runtime_measured and not old_runtime:
            _LOGGER.info("🎯 EWneo dual motor %s CH%d: Runtime measurement ACTIVATED! Position control now available", 
        ...
        """Parse dual motor state word with summary information for both motors."""
        # Extract motor 1 status code (bits 31-25)
        self._motor_status_codes[1] = (state_word >> 25) & 0x7F
        
        # Extract motor 1 tilt flag (bit 24)
        self._recent_tilts[1] = bool(state_word & (1 << 24))
        
        # Extract motor 2 status code (bits 23-17)
        self._motor_status_codes[2] = (state_word >> 17) & 0x7F
        
        # Extract motor 2 tilt flag (bit 16)
        self._recent_tilts[2] = bool(state_word & (1 << 16))
        
        # Bits 15-0 are reserved
        
        # Store old states for change detection
        old_states = self._motor_states.copy()
        ...
        for channel in [1, 2]:
            ...
            # Log state changes
            if (old_states[channel] != self._motor_states[channel] or 
                old_positions[channel] != self._positions[channel] or 
                self._motor_status_codes[channel] in [117, 118]):  # Always log measurement modes
                self._log_motor_state_change(channel)
    ...
    def _log_motor_full_state_change(self, channel: int) -> None:
        ...
        if not self._runtime_measured[channel]:
            ...
            _LOGGER.info("🏠 EWneo dual motor %s CH%d: %s (positionless mode)%s", 
            ...
            _LOGGER.info("🎯 EWneo dual motor %s CH%d: %s - Position: %s%s%s", 
    ...
    def _log_motor_state_change(self, channel: int) -> None:
        ...
        _LOGGER.info("EWneo dual motor %s CH%d: %s, Position=%s%s", 
                    self.serial_number[-6:], channel, self._motor_status_texts[channel], 
                    pos_text, flags_text)

    def get_motor_data(self) -> Dict[str, Any]:
        """Get comprehensive dual motor data."""
        # Base motor data
        motor_data = {
            ...
            "supports_position_control": {
                ch: self._runtime_measured[ch] for ch in [1, 2]
            },
            ...
            "positions_known": {
                ch: self._current_positions[ch] <= 100 for ch in [1, 2]
            },
            "is_calibrating": {
                ch: self._motor_status_codes[ch] in [117, 118] for ch in [1, 2]
            },
            "is_moving": {
                ch: self._motor_status_codes[ch] in [120, 121, 122, 123, 124, 125] for ch in [1, 2]
            }
        }
        ...
        if hasattr(self, '_target_positions'):
            motor_data.update({
                "target_positions": getattr(self, '_target_positions', {1: None, 2: None}).copy(),
                "tilt_measured": getattr(self, '_tilt_measured', {1: False, 2: False}).copy(),
                "terrace_function": getattr(self, '_terrace_function', {1: False, 2: False}).copy(),
                "stored_position": getattr(self, '_stored_position', {1: 0, 2: 0}).copy(),
                "auto_tilt": getattr(self, '_auto_tilt', {1: False, 2: False}).copy(),
                "supports_tilt_control": {
                    ch: getattr(self, '_tilt_measured', {1: False, 2: False})[ch] for ch in [1, 2]
                },
                "has_full_info": True
            })
        else:
    ...
    async def set_state(self, channel: int, command: str, position: Optional[int] = None) -> bool:
        ...
        if channel not in [1, 2] or command not in ["up", "down", "stop", "open", "close"]:
            return False
        ...
        try:
            if not self._runtime_measured[channel]:
                ...
                if position is not None:
                    _LOGGER.warning("EWneo dual motor %s CH%d: Position control not available - runtime measurement required", 
                                   self.serial_number[-6:], channel)
                    return False
                
                _LOGGER.info("Setting EWneo dual motor %s channel %d to %s (positionless mode)", 
                            self.serial_number[-6:], channel, command)
                ...
                _LOGGER.info("Setting EWneo dual motor %s channel %d to %s%s", 
                            self.serial_number[-6:], channel, command, 
            ...
            _LOGGER.error("Failed to set EWneo dual motor %s channel %d state: %s", 
    ...
    async def query_motor_state(self, channel: int, coordinator=None) -> bool:
        ...
        if channel not in [1, 2]:
            _LOGGER.error("Invalid motor channel %d for dual motor %s", channel, self.serial_number[-6:])
            return False
        ...
        try:
            ...
            query_mode = 2 if channel == 1 else 10
            
            _LOGGER.info("🔍 EWneo dual motor %s: Querying motor %d state (mode %d) for runtime measurement", 
            ...
            if not gateway_serial:
                _LOGGER.error("No gateway serial available for dual motor %s", self.serial_number[-6:])
                return False
    ...
    async def query_all_motors_state(self, coordinator=None) -> Dict[int, bool]:
        ...
        _LOGGER.info("🔍 EWneo dual motor %s: Queried all motors - Results: %s", 
...
def create_rx11_ewneo_dual_motor(
    ...
    **kwargs
    ...
    """Factory function to create EWneo dual motor."""
    return RX11EWneoDualMotor(
