"""Quad button transmitter implementation for RX11 transceiver."""
...
class RX11QuadButtonTransmitter(BaseTransmitter):
    """Quad button transmitter implementation for RX11 transceiver.
    
    Handles EW transmitters with four buttons (Button A, B, C, and D).
    """
    
    def __init__(self, *args, **kwargs):
        """Initialize quad button transmitter."""
        super().__init__(*args, device_type=DeviceType.EW_TRANSMITTER, 
                        subtype=DeviceSubtype.QUAD_BUTTON, 
                        operating_mode=OperatingMode.FOUR_BUTTON,
                        button_count=4, **kwargs)
    ...
    def process_telegram(self, telegram_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process incoming telegram for quad button transmitter."""
    ...
    def _handle_button_push(self, button: int, timestamp: datetime, telegram_data: Dict[str, Any]) -> None:
        ...
        if 0 <= button < 4:  # Button A (0), B (1), C (2), D (3)
            ...
            _LOGGER.debug("EW Quad Button Transmitter %s button %s pushed", 
    ...
    def _handle_button_release(self, button: int, timestamp: datetime, telegram_data: Dict[str, Any]) -> None:
        ...
        if 0 <= button < 4:  # Button A (0), B (1), C (2), D (3)
            ...
            button_label = self.button_labels[button]
            _LOGGER.debug("EW Quad Button Transmitter %s button %s released (duration: %.0fms)", 
    ...
    def is_three_button_mode(self) -> bool:
        """Check if this is used as a three-button transmitter."""
        # Three-button mode uses buttons A, B, C (0, 1, 2)
        return hasattr(self, '_three_button_mode') and self._three_button_mode
    
    def set_three_button_mode(self, enabled: bool) -> None:
        """Enable/disable three-button mode."""
        self._three_button_mode = enabled
        if enabled:
            self.operating_mode = OperatingMode.THREE_BUTTON
...
def create_rx11_quad_button_transmitter(
    ...
    **kwargs
    ...
    """Factory function to create quad button transmitter."""
    transmitter = RX11QuadButtonTransmitter(
        serial_number, 
        name=device_info.get('name'),
        **kwargs
    )
    
    # Check if this should be three-button mode
    button_count = device_info.get('button_count', 4)
    if button_count == 3:
        transmitter.set_three_button_mode(True)
